...
#      f.each do |gedline|
#        gedline = gedline.force_encoding("Windows-1252")
#        puts gedline.encode("utf-8")
...
#      f.rewind
      ...
      section = nil
      f.each do |gedline|
        gedline = gedline.force_encoding("Windows-1252") # FIXME

        level, tag, rest = gedline.chop.split(' ', 3)

        if level.to_i == 0
          handler(section_type, section) if section
          ...
          if section_type == 'FAM' && ['HUSB', 'WIFE', 'CHIL'].include?(tag)
            section.add_relation(tag, rest) if section
          else
    ...
    def handler(section_type, section)
      case section_type
      when 'INDI'
        @individuals.push section
      when 'FAM'
        @families.push section
      else
