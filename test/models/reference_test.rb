...
class ReferenceTest < ActiveSupport::TestCase
  ...
  test "note reference" do
    ...
    assert reference2 == note1.get_references[0], 'Association from reference to note does not work'
  end

  test "person reference" do
    reference1 = Reference.find(references(:reference1).id)
    person2 = Person.find(people(:person2).id)
    assert reference1 == person2.get_references[0], 'Association from reference to person does not work'
  end
