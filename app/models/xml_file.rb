...
#      puts child.name
      ...
      if not handle_person(v)
        raise StandardError, "Couldn't handle #{v.inspect}"
      end
      family = get_family(v)
...
#    puts "¤#{calling_name}¤"
  ...
  def make_family(family)
    parent_ids = []
    parent_ids.append(family[:father]) if not family[:father].nil?
    parent_ids.append(family[:mother]) if not family[:mother].nil?
    ...
    if parent_ids.length > 0
      begin
        ...
        puts "ERROR: Could not find parents in db: #{parent_ids}"
        parents = []
      end
    end
    ...
    if rel.nil?
      ...
      for parent in parents
        begin
          ...
          puts "ERROR: Parent not found! #{parent} #{family}" # FIXME: Log
        end        
      end
    end
